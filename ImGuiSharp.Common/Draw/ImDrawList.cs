namespace ImGui
{
    ...
    public class ImDrawList : IDisposable
    {
        ...
        internal void AddRectFilledMultiColor(ImVec2 a, ImVec2 c, uint col_upr_left, uint col_upr_right, uint col_bot_right, uint col_bot_left)
        {
            ...
            ImVec2 uv = GetCurrentTextureId().WhitePixel;
            PrimReserve(6, 4);
        ...
        internal void AddCircle(ImVec2 centre, float radius, uint col, int num_segments = 12, float thickness = 1.0f)
        {
            ...
            float a_max = ImMath.PI * 2.0f * (num_segments - 1.0f) / num_segments;
        ...
        internal void AddText(ImFont font, float font_size, ImVec2 pos, uint col, string text, int text_begin = 0, int text_end = -1, float wrap_width = 0.0f, ImVec4? cpu_fine_clip_rect = null)
        {
            ...
            if (cpu_fine_clip_rect.HasValue)
            {
                ...
                clip_rect.x = ImMath.Max(clip_rect.x, cfcr.x);
                clip_rect.y = ImMath.Max(clip_rect.y, cfcr.y);
                clip_rect.z = ImMath.Min(clip_rect.z, cfcr.z);
                clip_rect.w = ImMath.Min(clip_rect.w, cfcr.w);
            }
        ...
        internal void AddPolyline(ImVector<ImVec2> points, int num_points, uint col, bool closed, float thickness, bool anti_aliased)
        {
            ...
            ImVec2 uv = GetCurrentTextureId().WhitePixel;
            anti_aliased &= ImContext.AntiAliasedLines; //ImGui.Instance.Style.AntiAliasedLines;
        ...
        internal void AddConvexPolyFilled(ImVector<ImVec2> points, int num_points, uint col, bool anti_aliased)
        {
            var points_count = points.Size;
            ImVec2 uv = GetCurrentTextureId().WhitePixel;
            anti_aliased &= ImContext.AntiAliasedShapes; //ImGui.Instance.Style.AntiAliasedShapes;
            //if (ImGui::GetIO().KeyCtrl) anti_aliased = false; // Debug
        ...
        {
            if (_Path.Size == 0 || _Path[_Path.Size - 1].x != pos.x || _Path[_Path.Size - 1].y != pos.y)
                _Path.push_back(pos);
        }

        internal void PathFill(uint col)
        {
            AddConvexPolyFilled(_Path, _Path.Size, col, true);
            PathClear();
        }

        internal void PathStroke(uint col, bool closed, float thickness = 1.0f)
        {
            AddPolyline(_Path, _Path.Size, col, closed, thickness, true);
            PathClear();
        }
        ...
        internal void PathArcTo(ImVec2 centre, float radius, float amin, float amax, int num_segments = 10)
        {
            ...
            for (int i = 0; i <= num_segments; i++)
            {
                ...
                _Path.push_back(new ImVec2(centre.x + ImMath.Cosf(a) * radius, centre.y + ImMath.Sinf(a) * radius));
            }
        }
        ...
        internal void PathArcToFast(ImVec2 centre, float radius, int amin, int amax)
        {
            ...
            if (!circle_vtx_builds)
            {
                ...
                {
                    float a = ((float)i / (float)circle_vtx_count) * 2 * ImMath.PI;
                    circle_vtx[i].x = ImMath.Cosf(a);
                    circle_vtx[i].y = ImMath.Sinf(a);
                }
        ...
        internal void PathBezierCurveTo(ImVec2 p2, ImVec2 p3, ImVec2 p4, int num_segments = 0)
        {
            ...
            {
                // Auto-tessellated
                PathBezierToCasteljau(_Path, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, p4.x, p4.y, ImContext.CurveTessellationTol /*ImGui.Instance.Style.CurveTessellationTol*/, 0);
            }
            else
            {
        ...
        {
            float r = rounding;
            r = ImMath.Min(r, ImMath.Abs(b.x - a.x) * (((rounding_corners & (1 | 2)) == (1 | 2)) || ((rounding_corners & (4 | 8)) == (4 | 8)) ? 0.5f : 1.0f) - 1.0f);
            r = ImMath.Min(r, ImMath.Abs(b.y - a.y) * (((rounding_corners & (1 | 8)) == (1 | 8)) || ((rounding_corners & (2 | 4)) == (2 | 4)) ? 0.5f : 1.0f) - 1.0f);
...
#if USERCALLBACK
                || curr_cmd.Value.UserCallback != null
#endif
                )
            {
                AddDrawCmd();
...
#if USERCALLBACK
                && prev_cmd.Value.UserCallback == null
#endif
                )
                CmdBuffer.pop_back();
            else
        ...
        internal void PrimQuadUV(ImVec2 a, ImVec2 b, ImVec2 c, ImVec2 d, ImVec2 uv, uint col)
        {
            ImDrawIdx idx = (ImDrawIdx)_VtxCurrentIdx;
            IdxBuffer[_IdxWritePtr + 0] = idx; IdxBuffer[_IdxWritePtr + 1] = (ImDrawIdx)(idx + 1); IdxBuffer[_IdxWritePtr + 2] = (ImDrawIdx)(idx + 2);
            IdxBuffer[_IdxWritePtr + 3] = idx; IdxBuffer[_IdxWritePtr + 4] = (ImDrawIdx)(idx + 2); IdxBuffer[_IdxWritePtr + 5] = (ImDrawIdx)(idx + 3);
        ...
        {
            VtxBuffer[_VtxWritePtr++] = new ImDrawVert() { pos = pos, uv = uv, col = col };
            _VtxWritePtr++;
            _VtxCurrentIdx++;
        }
