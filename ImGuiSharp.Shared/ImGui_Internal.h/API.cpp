...
void ImGui::SetActiveID(uint id, ImGuiWindow* window = NULL)
{
	ImGuiState& g = *GImGui;
	g.ActiveId = id;
...
void ImGui::SetHoveredID(uint id)
{
	ImGuiState& g = *GImGui;
	g.HoveredId = id;
...
void ImGui::KeepAliveID(uint id)
{
	ImGuiState& g = *GImGui;
	if (g.ActiveId == id)
...
bool ImGui::ItemAdd(const ImRect& bb, const uint* id)
{
	ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::IsClippedEx(const ImRect& bb, const uint* id, bool clip_even_when_logged)
{
	ImGuiState& g = *GImGui;
...
bool ImGui::IsHovered(const ImRect& bb, uint id, bool flatten_childs)
{
	ImGuiState& g = *GImGui;
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, uint fill_col, bool border, float rounding)
{
	ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::RenderCheckMark(ImVec2 pos, uint col)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
...
static bool IsPopupOpen(uint id)
{
	ImGuiState& g = *GImGui;
...
static void ClosePopup(uint id)
{
	if (!IsPopupOpen(id))
		return;
	ImGuiState& g = *GImGui;
...
static bool BeginPopupEx(const char* str_id, ImGuiWindowFlags extra_flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	const uint id = window->GetID(str_id);
	if (!IsPopupOpen(id))
	{
...
bool ImGui::BeginPopupModal(const char* name, bool* p_opened, ImGuiWindowFlags extra_flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	const uint id = window->GetID(name);
	if (!IsPopupOpen(id))
	{
		ClearSetNextWindowData(); // We behave like Begin() and need to consume those values
		return false;
...
bool ImGui::BeginChild(uint id, const ImVec2& size, bool border, ImGuiWindowFlags extra_flags)
{
	char str_id[32];
...
bool ImGui::BeginChildFrame(uint id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
	ImGuiState& g = *GImGui;
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && !(flags & ImGuiWindowFlags_NoResize))
			{
				...
				const uint resize_id = window->GetID("#RESIZE");
				bool hovered, held;
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
	ImGuiState& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const uint id = window->GetID(horizontal ? "#SCROLLX" : "#SCROLLY");
	...
	// Render
	const uint grab_col = ImGui::GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab);
	if (horizontal)
...
bool ImGui::ButtonBehavior(const ImRect& bb, uint id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImGuiState& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const uint id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	...
	// Render
	const uint col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
...
static bool CloseWindowButton(bool* p_opened)
{
	...
	const uint id = window->GetID("#CLOSE");
	...
	// Render
	const uint col = ImGui::GetColorU32((held && hovered) ? ImGuiCol_CloseButtonActive : hovered ? ImGuiCol_CloseButtonHovered : ImGuiCol_CloseButton);
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	...
	const uint id = window->GetID("#image");
	ImGui::PopID();
...
bool ImGui::TreeNodeBehaviorIsOpened(uint id, ImGuiTreeNodeFlags flags)
{
...
bool ImGui::CollapsingHeader(const char* label, const char* str_id, bool display_frame, bool default_open)
{
	...
	// Render
	const uint col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
uint ImGui::GetID(const char* str_id)
{
	return GImGui->CurrentWindow->GetID(str_id);
}

uint ImGui::GetID(const char* str_id_begin, const char* str_id_end)
{
	return GImGui->CurrentWindow->GetID(str_id_begin, str_id_end);
}

uint ImGui::GetID(const void* ptr_id)
{
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, uint id, int decimal_precision)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, uint id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
...
bool ImGui::DragBehavior(const ImRect& frame_bb, uint id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
	ImGuiState& g = *GImGui;
	const ImGuiStyle& style = g.Style;

	// Draw frame
	const uint frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float(*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
	...
	const uint col_base = GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
	const uint col_hovered = GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered : ImGuiCol_PlotHistogramHovered);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
	{
		...
		if (edit_state.StbState.select_start != edit_state.StbState.select_end)
		{
			...
			uint bg_color = GetColorU32(ImGuiCol_TextSelectedBg);
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
	...
	const uint id = ImGui::GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
...
bool ImGui::ColorButton(const ImVec4& col, bool small_height, bool outline_border)
{
	...
	ImGuiState& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const uint id = window->GetID("#colorbutton");
...
static float GetDraggedColumnOffset(int column_index)
{
	...
	IM_ASSERT(g.ActiveId == window->DC.ColumnsSetID + uint(column_index));
...
float ImGui::GetColumnOffset(int column_index)
{
	...
	if (g.ActiveId)
	{
		const uint column_id = window->DC.ColumnsSetID + uint(column_index);
...
void ImGui::Columns(int columns_count, const char* id, bool border)
{
	...
	if (window->DC.ColumnsCount != columns_count && window->DC.ColumnsCount != 1 && window->DC.ColumnsShowBorders && !window->SkipItems)
	{
		...
		for (int i = 1; i < window->DC.ColumnsCount; i++)
		{
			...
			const uint column_id = window->DC.ColumnsSetID + uint(i);
			...
			const uint col = GetColorU32(held ? ImGuiCol_ColumnActive : hovered ? ImGuiCol_ColumnHovered : ImGuiCol_Column);
